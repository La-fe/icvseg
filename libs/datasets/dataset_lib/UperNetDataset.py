...
import libs.utils.UperNetData as torchdata
import cv2
...
class BaseDataset(torchdata.Dataset):
    ...
    def parse_input_list(self, odgt, max_sample=-1, start_idx=-1, end_idx=-1):
        # original odgt file should be changed by txt file
        ...
        #     self.list_sample = [json.loads(x.rstrip()) for x in open(odgt, 'r')]
            ...
            for i in range(len(files)):
                ...
                files[i][2] = int(files[i][2])
                files[i][3] = int(files[i][3])
            self.list_sample = files
...
class TrainDataset(BaseDataset):
    ...
    def _get_sub_batch(self):
        while True:
            ...
            if this_sample[2] > this_sample[3]:
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.batch_per_gpu):
            ...
            img_height, img_width = batch_records[i][2], batch_records[i][3]
            this_scale = min(
        ...
        for i in range(self.batch_per_gpu):
            ...
            image_path = os.path.join(self.root_dataset, this_record[0])
            segm_path = os.path.join(self.root_dataset, this_record[1])
